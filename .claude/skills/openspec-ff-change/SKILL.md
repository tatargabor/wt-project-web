...
- Add a `### Manual: <task-id> — <title>` section after each `[?]` task with:
  - **What to do:** numbered step-by-step instructions
  - Relevant URLs or links
  - **Input:** expected key name (for input tasks)
  - **Format:** expected value format
  - **Target:** where the value will be stored (e.g., `.env.local`)
...
**Format:** `sk_test_...` or `sk_live_...` (32+ characters)
**Target:** `.env.local`
```
